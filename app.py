...
if not st.session_state.authenticated:
    ...
    st.stop() # Parol kiritilmaguncha kod shu yerda to'xtaydi
...
KPI_DATABASE_ID = st.secrets.get("KPI_DATABASE_ID", "") # YANGI QO'SHILDI
...
kpi_recruiter_options = list(recruiter_dict.keys())
kpi_recruiter_options.sort()
...
# ==========================================
# 4. 📋 SAHIFA NAVIGATSIYASI (SIDEBAR)
# ==========================================
st.sidebar.markdown("### 🦅 909 RA Menu")
page = st.sidebar.radio("Bo'limni tanlang:", ["🚀 Drayver Onboarding", "📊 Recruiter KPI (Daily Log)"])

# ---------------------------------------------------------
# SAHIFA 1: DRAYVER ONBOARDING (ABDULLAYEV)
# ---------------------------------------------------------
if page == "🚀 Drayver Onboarding":
...
# ---------------------------------------------------------
# SAHIFA 2: KUNLIK NAZORAT (KARIMOV)
# ---------------------------------------------------------
elif page == "📊 Recruiter KPI (Daily Log)":
    st.title("📈 Recruiter Daily Performance")
    st.markdown("Bugungi ish yuzasidan aniq raqamlarni kiriting. Har bir raqam oylik daromadga ta'sir qiladi.")
    st.markdown("---")
    
    with st.form("kpi_daily_log", clear_on_submit=True):
        kpi_recruiter_name = st.selectbox("Recruiter Name *", kpi_recruiter_options)
        report_date = st.date_input("Sana *")
        
        col1, col2, col3 = st.columns(3)
        with col1:
            calls = st.number_input("📞 Outbound Calls", min_value=0, step=1, help="Kun davomida qilingan jami qo'ng'iroqlar")
        with col2:
            leads = st.number_input("🎯 Qualified Leads", min_value=0, step=1, help="Talabga mos kelgan drayverlar")
        with col3:
            cdls = st.number_input("🪪 CDL Collected", min_value=0, step=1, help="Hujjatlari to'liq yig'ilgan drayverlar")
            
        notes = st.text_area("✍️ Kunlik xulosa (Qiyinchiliklar, e'tirozlar)")
        
        submitted_kpi = st.form_submit_button("📊 HISOBOTNI YUBORISH")
        
        if submitted_kpi:
            if calls == 0:
                st.error("❌ Xato: Outbound Calls 0 bo'lishi mumkin emas. Ishlagan bo'lsangiz raqam kiriting!")
            else:
                conversion_rate = round((cdls / calls) * 100, 1)
                
                kpi_payload = {
                    "parent": {"database_id": KPI_DATABASE_ID},
                    "properties": {
                        "Recruiter": {"title": [{"text": {"content": kpi_recruiter_name}}]}, 
                        "Date": {"date": {"start": str(report_date)}},
                        "Outbound Calls": {"number": calls},
                        "Qualified Leads": {"number": leads},
                        "CDL Collected": {"number": cdls},
                        "Notes": {"rich_text": [{"text": {"content": notes}}]}
                    }
                }
                
                try:
                    kpi_res = requests.post("https://api.notion.com/v1/pages", headers=headers, json=kpi_payload)
                    if kpi_res.status_code in [200, 201]:
                        st.success(f"✅ {kpi_recruiter_name}, hisobot qabul qilindi va tizimga yozildi!")
                        st.info(f"Sizning bugungi CDL konversiya darajangiz: **{conversion_rate}%**")
                    else:
                        st.warning(f"⚠️ API Xatosi: {kpi_res.text}")
                except Exception as e:
